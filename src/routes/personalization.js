 ...
 *   get:
 *     summary: 개인화 서비스 개요
 *     description: 사용자 맞춤형 추천 및 개인화 서비스의 개요 정보와 사용 가능한 엔드포인트를 반환합니다.
 *     tags: [Personalization]
 *     responses:
 *       200:
 *         description: 개인화 서비스 개요 정보
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                     description:
 *                       type: string
 *                       example: "사용자 맞춤형 추천 및 개인화 서비스"
 *                     availableEndpoints:
 *                       type: array
 *                       items:
 *                         type: string
 *                       example: ["/profile", "/recommendations", "/preferences", "/history", "/insights"]
 *                     lastUpdate:
 *                       type: string
 *                       format: date-time
  ...
  try {
    res.json({
      ...
      data: {
        ...
        description: '사용자 맞춤형 추천 및 개인화 서비스',
        availableEndpoints: [
          '/profile',
          '/recommendations',
          '/preferences',
    ...
    logger.error('개인화 서비스 개요 조회 실패:', error);
    res.status(500).json({
      success: false,
